...
export async function buildVisualDiffDocument(
  ...
  const heading = showHeading
    ? `<h2>${escapeHtml(options.heading?.replaceAll('_', ' ') || (options.wikiHost === 'he.wikipedia.org' ? 'תוכן שהשתנה' : options.wikiHost === 'th.wikipedia.org' ? 'เนื้อหาที่เปลี่ยนแปลง' : options.wikiHost === 'de.wikipedia.org' ? 'Geänderter Inhalt' : 'Changed content'))}</h2>`
    : ''
